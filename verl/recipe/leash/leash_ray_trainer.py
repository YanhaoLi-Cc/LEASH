...
class RayLeashTrainer(RayPPOTrainer):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initialize leash manager if enabled
        self.use_leash = self.config.algorithm.get("use_leash", False)
        print(f"\033[93mDAPOTrainer: use_leash = {self.use_leash}\033[0m")
        self.leash_manager = None
        
        if self.use_leash:
            from .leash_reward_manager import LeashRewardManager
            
            leash_config = self.config.algorithm.leash_config
            self.leash_manager = LeashRewardManager(
                target_length=leash_config.target_length,
                lambda_init=leash_config.lambda_init,
                lambda_lr=leash_config.lambda_lr,
                lambda_max=leash_config.lambda_max,
                lambda_min=leash_config.lambda_min,
                leash_type=leash_config.leash_type,
            )
            if self.leash_manager is not None:
                print("\033[91mLoad LeashRewardManager Successful\033[0m")
    ...
    def fit(self):
        ...
        if self.global_steps != 0 and self.use_leash:
            self.leash_manager.load_state(os.path.join(self.config.trainer.default_local_dir,f"{self.global_steps}_leash_manager_state.pt"))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        # Compute response_mask before applying leash
                        if "response_mask" not in new_batch.batch:
                            new_batch.batch["response_mask"] = compute_response_mask(new_batch)

                        # Apply Lagrangian leash if enabled
                        if self.use_leash and self.leash_manager is not None:
                            leash_result = self.leash_manager.compute_constrained_reward(
                                new_batch,
                            ...
                            new_batch.batch["token_level_scores"] = leash_result["reward_tensor"]
                            
                            # Update metrics with leash info
                            leash_metrics = self.leash_manager.get_metrics()
                            print(f"\033[92mLeash metrics: {list(leash_metrics.keys())}\033[0m")
                            metrics.update(leash_metrics)
                    ...
                    if self.config.trainer.save_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.save_freq == 0):
                        with _timer("save_checkpoint", timing_raw):
                            ...
                            if self.use_leash:                            
                                self.leash_manager.save_state(os.path.join(self.config.trainer.default_local_dir,f"{self.global_steps}_leash_manager_state.pt"))
