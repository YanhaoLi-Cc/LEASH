...
class LeashRewardManager:
    ...
    def __init__(
        ...
        lambda_init: float = 0.01,  # Adjusted for ratio-based leash
        ...
        leash_type: str = "average",  # "average" or "max"
    ):
        self.target_length = target_length
        self.lambda_lr = lambda_lr
        self.lambda_max = lambda_max
        self.lambda_min = lambda_min
        self.leash_type = leash_type
    ...
    def compute_accuracy(self, original_rewards: torch.Tensor, response_lengths: list) -> float:
        ...
        Args:
            original_rewards: Original reward tensor before leash modification
    ...
    def compute_constrained_reward(
        ...
        for i in range(len(reward_tensor)):
            ...
            # Compute leash violation for this sample (ratio-based)
            violation_ratio = valid_response_length / self.target_length - 1.0
            leash_violations.append(violation_ratio)
        ...
        # Compute leash statistics
        avg_length = np.mean(response_lengths)
        max_length = np.max(response_lengths)
        
        # Compute leash violation based on type (ratio-based)
        if self.leash_type == "average":
            # Average leash: $g = \frac{\mathbb{E}[|y|]}{L_{target}} - 1$
            current_leash_violation = avg_length / self.target_length - 1.0
        else:  # "max"
            # Max leash: $g = \frac{\max(|y|)}{L_{target}} - 1$
            current_leash_violation = max_length / self.target_length - 1.0
            
        # Update Lagrange multiplier directly based on current leash violation
        # Formula: $\lambda^{(t+1)} = \text{clip}(\lambda^{(t)} + \eta_\lambda \cdot g^{(t)}, \lambda_{min}, \lambda_{max})$
        self.lagrange_multiplier += self.lambda_lr * current_leash_violation
        ...
        self.stats["leash_violation"].append(current_leash_violation)
        ...
        penalties = [self.lagrange_multiplier * v for v in leash_violations]
        ...
        if return_dict:
            return {
                ...
                "reward_extra_info": {
                    ...
                    "leash_violations": leash_violations,
    ...
    def get_metrics(self) -> Dict[str, float]:
        """Get current leash optimization metrics."""
        metrics = {}
        
        # Use the last recorded values from stats
        if self.stats["avg_length"]:
            metrics["leash/avg_length"] = self.stats["avg_length"][-1]
            metrics["leash/std_length"] = self.stats["std_length"][-1]
            metrics["leash/min_length"] = self.stats["min_length"][-1]
            metrics["leash/max_length"] = self.stats["max_length"][-1]
            metrics["leash/p95_length"] = self.stats["p95_length"][-1]
            
            # Leash satisfaction metrics
            metrics["leash/satisfaction_rate"] = self.stats["satisfaction_rate"][-1]
            metrics["leash/num_violations"] = self.stats["num_violations"][-1]
            
            # Distance from target (relative)
            if self.stats["avg_length"][-1] > 0:
                metrics["leash/avg_relative_distance"] = abs(self.stats["avg_length"][-1] / self.target_length - 1.0)
            
        # Penalty statistics
        if self.stats["penalty_magnitude"]:
            metrics["leash/avg_penalty"] = self.stats["penalty_magnitude"][-1]
            metrics["leash/penalty_active_rate"] = self.stats["penalty_active_rate"][-1]
            if self.stats["avg_active_penalty"]:
                metrics["leash/avg_active_penalty"] = self.stats["avg_active_penalty"][-1]
                    
        # Lambda tracking
        if self.stats["lagrange_multiplier"]:
            metrics["leash/lambda"] = self.stats["lagrange_multiplier"][-1]
            if self.stats["lambda_change_rate"]:
                metrics["leash/lambda_change_rate"] = self.stats["lambda_change_rate"][-1]
                
        # Violation statistics
        if self.stats["leash_violation"]:
            metrics["leash/avg_violation_ratio"] = self.stats["leash_violation"][-1]
            
        # Add current values
        metrics["leash/target_length"] = self.target_length

        # Training accuracy metrics
        if self.stats["training_acc"]:
            metrics["leash/training_acc"] = self.stats["training_acc"][-1]
    ...
    def get_summary_string(self) -> str:
        ...
        if "leash/avg_length" in metrics:
            summary_parts.append(
                f"Avg Length: {metrics['leash/avg_length']:.1f} "
                f"(target: {self.target_length}, "
                f"sat_rate: {metrics.get('leash/satisfaction_rate', 0):.2%})"
            )
            
        if "leash/lambda" in metrics:
            summary_parts.append(f"λ: {metrics['leash/lambda']:.4f}")
            
        if "leash/avg_penalty" in metrics:
            summary_parts.append(
                f"Penalty: {metrics['leash/avg_penalty']:.3f} "
                f"(active: {metrics.get('leash/penalty_active_rate', 0):.2%})"
            )

        if "leash/training_acc" in metrics:
            summary_parts.append(f"Acc: {metrics['leash/training_acc']:.2%}")
    ...
    def save_state(self, file_path: str):
        ...
        try:
            ...
            print(f"\033[92m✅ LeashRewardManager state saved to {file_path}\033[0m")
        except Exception as e:
            print(f"\033[91m❌ Error saving LeashRewardManager state: {e}\033[0m")
    ...
    def load_state(self, file_path: str):
        ...
        try:
            ...
            print(f"\033[92m✅ LeashRewardManager state loaded from {file_path}\033[0m")
            print(f"  - Resumed λ: {self.lagrange_multiplier:.4f}")

        except Exception as e:
            print(f"\033[91m❌ Error loading LeashRewardManager state from {file_path}: {e}. \033[0m")
            raise RuntimeError(f"Failed to load LeashRewardManager state from {file_path}: {e}")
